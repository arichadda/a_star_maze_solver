...
class MazeworldProblem:
    ...
    def get_valid_moves(self, current_x, current_y, goal_x, goal_y):
        ...
        # get the legal moves by maze size and wall locations
        if current_x is not goal_x or current_y is not goal_y:
            if self.maze.is_floor(current_x + 1, current_y) and current_x + 1 <= self.maze.width:
                moves_list.append((current_x + 1, current_y))
            if self.maze.is_floor(current_x, current_y + 1) and current_y + 1 <= self.maze.height:
                moves_list.append((current_x, current_y + 1))
            if self.maze.is_floor(current_x - 1, current_y) and current_x - 1 > -1:
                moves_list.append((current_x - 1, current_y))
            if self.maze.is_floor(current_x, current_y - 1) and current_y - 1 > -1:
                moves_list.append((current_x, current_y - 1))
...
# A bit of test code. You might want to add to it to verify that things work as expected.
